...
class Backtester:
    ...
    def _check_exit_conditions(self, timestamp: pd.Timestamp, row: pd.Series):
        ...
        for position, exit_price, reason in positions_to_close:
            self._close_position(position, timestamp, exit_price)
    ...
    def _close_position(self, position: Position, timestamp: pd.Timestamp, exit_price: float):
        """Close position"""
        # Calculate PnL
    ...
    def _close_all_positions(self, timestamp: pd.Timestamp, current_price: float, reason: str = ""):
